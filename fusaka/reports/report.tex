...
\noindent This report outlines the results of the "Fusaka" investigation, aiming to align the Filecoin Virtual Machine (FVM) with Ethereum's upcoming Hard Fork. The upgrade focuses on scaling data availability (PeerDAS) and enhancing execution capabilities (secp256r1, CLZ). Our analysis concludes that while FVM should adopt the execution-layer improvements to maintain EVM equivalence for developers, the data availability and gas-layer changes are architecturally incompatible with Filecoin's native storage and metering models. We recommend implementing the three adopted features as \textbf{three separate Filecoin Improvement Proposals (FIPs)} to facilitate modular governance and activation.
